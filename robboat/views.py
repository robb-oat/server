...
def webhook(request):
    ...
    return JsonResponse({
        ...
        'instruction': instruction,import re
# s = set()
# with open('uri_perms.txt') as uri_perms:
    # for line in uri_perms:
        # xs, ys, zs = line.split()
        # if re.match(r'bioshare\.bb\.psu\.edu.*', zs):
            # z = zs.replace('bioshare.bb.psu.edu', '')
            # if z.startswith('/download/'):
                # z = z.replace('/download/', '')
            # if z.startswith('/files/'):
                # z = z.replace('/files/', '')
                # if z.startswith('/auth/'):
                    # z = z.replace('/auth/', '')
                    # if z.startswith('.*/auth/'):
                        # z = z.replace('.*/auth/', '')
                    # else:
